...
def make_goldbach_partition(primes, limit):
    goldbach = [()] * (limit+1)
    for i in range(len(primes)):
        for j in range(i, len(primes)):
            sumprime = primes[i] + primes[j]
            if sumprime > limit:
                break
            if goldbach[sumprime]:
                a, b = goldbach[sumprime]
                if b - a > primes[j] - primes[i]:
                    goldbach[sumprime] = primes[i], primes[j]
            else:
                goldbach[sumprime] = primes[i], primes[j]
...
goldbach = make_goldbach_partition(primes, LIMIT)

TestCase = int(stdin.readline())
for _ in range(TestCase):
    odd_num = int(stdin.readline())
    print(" ".join(map(str, goldbach[odd_num])))
